...
// Human-readable stage metadata (step drives the progress bar)
const STATUS_STAGE: Record<string, { label: string; color: string; step: number }> = {
  NEW:      { label: 'Waiting for responses', color: 'bg-blue-50 text-blue-600',    step: 0 },
  QUOTED:   { label: 'Quotes received',       color: 'bg-green-50 text-green-700',  step: 1 },
  CHATTING: { label: 'In discussion',         color: 'bg-purple-50 text-purple-600', step: 1 },
  ACCEPTED: { label: 'Booked',               color: 'bg-black text-white',          step: 2 },
  DECLINED: { label: 'Declined',             color: 'bg-gray-100 text-gray-400',    step: -1 },
  EXPIRED:  { label: 'Expired',              color: 'bg-gray-100 text-gray-400',    step: -1 },
  COMPLETED:{ label: 'Completed',            color: 'bg-gray-100 text-gray-500',    step: 3 },
};
...
// Returns the primary CTA for a job card
...
// Lightweight 4-step progress bar shown on each job card
function JobProgressBar({ step }: { step: number }) {
  if (step < 0) return null;
  return (
    <div className="flex items-center gap-1 my-3">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= step ? 'bg-black' : 'bg-gray-100'}`} />
      ))}
    </div>
  );
}
    ...
    Promise.all([
      ...
      for (const b of books) {
        if (b.status === 'SCHEDULED')
          ...
          notifs.push({ id: `d-${b.id}`, type: 'status', title: 'Job completed', body: `€${b.totalAmount?.toFixed(0)} · tap to leave a review`, time: b.createdAt, href: `/bookings/${b.id}` });
      }
      ...
      <aside className="w-16 lg:w-56 bg-white border-r border-gray-100 flex flex-col sticky top-0 h-screen shrink-0">
        ...
        <nav className="flex-1 px-2 lg:px-3 py-4 space-y-1 overflow-y-auto">
          <Link href="/dashboard" className="flex items-center gap-3 px-2 lg:px-3 py-2.5 rounded-xl font-semibold text-sm bg-black text-white">
            <LayoutDashboard className="w-4.5 h-4.5 w-5 h-5 shrink-0" /><span className="hidden lg:block">Dashboard</span>
          </Link>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-7">
              <div className="flex items-center gap-3.5">
                <div className="w-11 h-11 rounded-2xl bg-black text-white flex items-center justify-center text-sm font-bold shrink-0 select-none">
                  {getInitials(session.user?.name)}
                </div>
                <div>
                  <h1 className="text-xl font-bold tracking-tight">Hello, {firstName}</h1>
                  <p className="text-sm text-gray-400 mt-0.5">Describe your task and receive quotes from trusted pros.</p>
                </div>
              </div>
              <Link
                href="/requests/new"
            ...
            {/* ── Contextual banners (shown only when relevant) ── */}
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-2xl px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-blue-900">
                      {nextBooking.provider?.user?.name ?? 'Your pro'} is coming {bookingDateLabel(nextBooking.scheduledAt)} at{' '}
                    ...
                    <p className="text-xs text-blue-500 mt-0.5 truncate">
                      {nextBooking.quote?.request?.category?.name ?? 'Service'} · €{nextBooking.totalAmount?.toFixed(0)}
                    </p>
                  </div>
                  <Link href={`/bookings/${nextBooking.id}`} className="shrink-0 bg-blue-600 text-white px-3 py-1.5 rounded-xl text-xs font-bold hover:bg-blue-700 transition-colors">
                    Details
                  </Link>
                </div>
              )}
              ...
              {totalQuotes > 0 && quotedReqs[0] && (
                <div className="bg-green-50 border border-green-100 rounded-2xl px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-green-600 mt-0.5">Compare prices and book the right pro.</p>
                  </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                  ...
                  {requests.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {requests.slice(0, 6).map(req => {
                        const stage      = STATUS_STAGE[req.status] ?? { label: req.status, color: 'bg-gray-100 text-gray-400', step: 0 };
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <span className={`shrink-0 text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full ${stage.color}`}>
                                {stage.label}
                              </span>
                            </div>

                            {/* Job description + location */}
                            <p className="font-semibold text-sm mt-2.5 mb-1 line-clamp-1">{req.description}</p>
                            <p className="flex items-center gap-1 text-xs text-gray-400">
                              <MapPin className="w-3 h-3 shrink-0" /> {req.address}
                            </p>

                            {/* 4-step progress bar */}
                            <JobProgressBar step={stage.step} />

                            {/* Footer: quote avatars + primary action */}
                                ...
                                {quoteCount > 0 ? (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-gray-500 font-medium">
                                      {quoteCount} pro{quoteCount > 1 ? 's' : ''} responded
                                    </span>
                                  </div>
                                ) : isActive ? (
                ...
                {/* Browse Services — supportive, not competing */}
                <section>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {POPULAR_SERVICES.map(({ label, slug, emoji }) => (
                      <Link
                        ...
                        className="bg-white border border-gray-100 rounded-2xl p-3 flex flex-col items-center gap-1.5 text-center hover:border-gray-300 hover:shadow-sm transition-all group"
                      >
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                  ...
                  <p className="font-bold text-sm mb-1">Need something done?</p>
                  <p className="text-xs text-gray-400 mb-4 leading-relaxed">
                    Post a job in 2 minutes and receive quotes from verified local pros.
                  </p>
                ...
                {/* Recommended Pros — supportive, lighter emphasis */}
                <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-sm">Recommended Pros</h3>
                    ...
                    <div className="space-y-1">
                      {[
                        ...
                        <Link
                          ...
                          className="flex items-center gap-3 px-2 py-2.5 rounded-xl hover:bg-gray-50 transition-all group"
                        >
                          <span className="text-base leading-none">{s.emoji}</span>
                          <span className="text-sm font-semibold text-gray-600 group-hover:text-black transition-colors flex-1">{s.label}</span>
                          <ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-black transition-colors shrink-0" />
                        </Link>
                      ))}
                    </div>
                  ) : (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1 mt-0.5">
                              ...
                              <span className="text-xs text-gray-500">{pro.ratingAvg?.toFixed(1) ?? '—'}</span>
                              <span className="text-[10px] text-gray-300 mx-0.5">·</span>
                              <span className="text-[10px] text-gray-400 truncate">{pro.categories?.[0]?.name ?? 'Pro'}</span>
                            </div>
                          </div>
                          {pro.isVerified && <ShieldCheck className="w-4 h-4 text-blue-400 shrink-0" />}
                        </Link>
                      ))}
                    </div>
                  )}
