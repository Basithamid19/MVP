...
export default function ProviderProfileClient({
  ...
  useEffect(() => {
    // Always fetch reviews client-side (they're paginated and non-critical for
    // above-the-fold render). Only re-fetch the provider when we don't have
    ...
    const fetchAll = async () => {
      try {
        const [provData, revData] = await Promise.all([
          initialProvider
            ? Promise.resolve(initialProvider)
            : fetchJsonOr<any>(`/api/providers?id=${id}`, null),
          fetch(`/api/reviews?providerId=${id}`).then(r => r.ok ? r.json() : []).catch(() => []),
        ]);
        if (cancelled) return;
        if (!initialProvider) setProvider(provData?.id ? provData : null);
