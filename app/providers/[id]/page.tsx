  ...
  return (
    <CustomerLayout maxWidth="max-w-5xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-bold text-lg flex-1">Professional Profile</h1>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2 flex-wrap">
                  <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-ink">{provider.user.name}</h2>
                  {provider.isVerified && (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {qualityIndicators.map((qi, i) => (
                <div key={i} className={`p-4 rounded-card border text-center ${qi.positive ? 'bg-trust-surface border-trust-edge' : 'bg-surface-alt border-border-dim'}`}>
                  ...
                  <p className="text-[11px] text-ink-dim mt-0.5">{qi.label}</p>
                </div>
              ))}
            </div>
          </div>
                    ...
                    <div className="text-right">
                      {offering?.price ? (
                        <>
                          ...
                          <div className="text-lg font-bold">€{Number(offering.price).toFixed(0)}<span className="text-ink-dim text-xs font-medium ml-1">/hr</span></div>
                        </>
                      ) : (
            ...
            <div className="grid grid-cols-7 gap-1.5 mb-4">
              {Object.entries(AVAILABILITY).map(([day, avail]) => (
                <div key={day} className="text-center">
                  <p className="text-[11px] font-bold text-ink-dim uppercase mb-1.5">{avail.label}</p>
                  <div className={`rounded-input py-3 text-[11px] font-bold uppercase tracking-wide ${
              ...
              <div className="space-y-2 mb-6 p-4 bg-surface-alt rounded-card">
                {ratingDistribution.map(({ star, count }) => (
                  <div key={star} className="flex items-center gap-3">
                    ...
                    <Star className="w-3.5 h-3.5 text-brand fill-current shrink-0" />
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-8 rounded-panel border border-border-dim shadow-elevated sticky top-24">
