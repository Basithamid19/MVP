...
function BrowseContent({ initialProviders }: { initialProviders: any[] }) {
  ...
  // On first render, only trust the server data if it matches the URL category.
  // Otherwise the effect below refetches once for the correct slug.
  ...
  // Track whether the current provider list matches the current category —
  // the server rendered `initialCategory`, so any switch triggers a refetch.
  const [loadedCategory, setLoadedCategory] = useState<string>(initialCategory);
  ...
  useEffect(() => {
    ...
    // Uses CDN-cached /api/providers, retried on cold-start / 5xx.
    fetchJsonOr<any[]>(`/api/providers?${params.toString()}`, [])
      .then(data => {
        if (cancelled) return;
...
export default function BrowseClient({ initialProviders = [] }: { initialProviders?: any[] }) {
  return (
    <Suspense fallback={
      ...
      <BrowseContent initialProviders={initialProviders} />
    </Suspense>
  );
}
