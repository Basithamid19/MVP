...
export default function OnboardingPage() {
  ...
  // Step 5 ("Done") is the success screen, not a stepper stop — the stepper
  // shows the four steps the provider actually fills in.
  const STEPS = [
    { key: 'identity',    label: t.onboarding.stepIdentity },
    { key: 'business',    label: t.onboarding.stepBusiness },
    { key: 'credentials', label: t.onboarding.stepCredentials },
    { key: 'selfie',      label: t.onboarding.stepSelfie },
  ];
  ...
  return (
    <div className="min-h-screen bg-canvas">
      {/* Header */}
      <header className="bg-card border-b border-border-dim sticky top-0 z-40">
        <div className="max-w-2xl mx-auto px-4 h-16 flex items-center gap-4">
          {step > 1 && step < 5 ? (
            <button
              onClick={() => setStep(s => s - 1)}
              className="p-2 hover:bg-surface-alt rounded-full transition-colors shrink-0"
          ...
          {/* Same stepper as the request wizard — connectors follow STEPS.length */}
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 pb-32">
            ...
            <div className="space-y-5">
              <div>
                ...
                <Input
                  ...
                  aria-label={t.onboarding.fullLegalName}
              ...
              <div>
                ...
                <Input
                  ...
                  aria-label={t.onboarding.phoneNumber}
                  className="py-4 text-base"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  aria-label={t.onboarding.nationalIdLabel}
                  className="py-4 text-base"
                />
              </div>
              <div className="p-4 bg-info-surface border border-info-edge rounded-card text-xs text-info leading-relaxed">
              ...
              {BUSINESS_TYPES.map(bt => {
                ...
                return (
                  <button
                    ...
                    className={`relative w-full text-left p-5 rounded-card border-2 transition-all ${
                      selected
                        ? 'bg-brand text-white border-brand shadow-elevated'
                        : 'bg-card border-border text-ink-sub hover:border-border-dim'
                    }`}
                  >
                    {selected && <Check className="absolute top-4 right-4 w-4 h-4 text-white" strokeWidth={3} />}
                    <p className="font-bold mb-0.5 pr-6">{bt.label}</p>
                    <p className={`text-xs ${selected ? 'text-white/70' : 'text-ink-dim'}`}>{bt.desc}</p>
                  </button>
                );
              })}
            </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    aria-label={t.onboarding.vatNumber}
                    className="py-4 text-base"
                  />
                </div>
              </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              {DOC_TYPES.map(doc => {
                ...
                return (
                  <div key={doc.id} className={`p-4 rounded-card border transition-all ${
                    uploaded?.uploaded ? 'border-trust-edge bg-trust-surface' : 'border-border bg-card'
                  }`}>
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <p className="font-bold text-sm text-ink">
                          {doc.label}
                          {doc.required && <span className="ml-1 text-danger">*</span>}
                        </p>
                        {!doc.required && <p className="text-xs text-ink-dim">{t.onboarding.optionalTrust}</p>}
                      </div>
                      {uploaded?.uploaded && <CheckCircle2 className="w-5 h-5 text-trust shrink-0" />}
                    </div>
              ...
              <div className="space-y-4">
                <div className="relative w-full aspect-[4/3] rounded-panel overflow-hidden border border-border">
                  ...
                  <button
                    ...
                    aria-label={t.common.delete}
                    className="absolute top-3 right-3 w-8 h-8 bg-ink/60 hover:bg-ink rounded-full flex items-center justify-center transition-colors"
                  >
                    <X className="w-4 h-4 text-white" />
                  </button>
                </div>
                <div className="p-4 bg-trust-surface border border-trust-edge rounded-card flex items-center gap-3">
                  <CheckCircle2 className="w-5 h-5 text-trust shrink-0" />
                  <div>
                    <p className="font-bold text-sm text-trust">{t.onboarding.lookingGood}</p>
                    <p className="text-xs text-ink-sub">{t.onboarding.reviewIn24h}</p>
                  </div>
                </div>
              </div>
            ) : (
        ...
        {step === 5 && (
          <div className="text-center py-12">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mb-3">{t.onboarding.doneTitle}</h1>
            ...
            <div className="bg-card rounded-panel border border-border-dim p-6 text-left max-w-sm mx-auto mb-8 shadow-card space-y-3">
              {[
                ...
                <div key={item.label} className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${item.done ? 'bg-trust' : 'bg-border'}`}>
            ...
            <Link href="/provider/dashboard" className={buttonVariants({ variant: 'primary', size: 'xl' })}>
      ...
      {/* Bottom bar */}
      {step < 5 && (
        <div className="fixed bottom-0 left-0 right-0 bg-card border-t border-border-dim p-4">
          <div className="max-w-2xl mx-auto">
            {finishError && step === 4 && (
              <div className="px-4 py-3 mb-3 bg-caution-surface border border-caution-edge rounded-card text-sm font-medium text-caution leading-relaxed">
                {finishError}
              </div>
            )}
            {step < 4 ? (
              ...
              <Button
                ...
                className="w-full"
              >
                {!saving && <Shield className="w-4 h-4" />} {t.onboarding.submitForReview}
              </Button>
            )}
          </div>
        </div>
