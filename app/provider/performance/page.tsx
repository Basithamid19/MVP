    ...
    <div className="p-6 lg:p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Track your quality metrics and standing</p>
        </div>
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold ${
          ratingAvg >= 4.5 ? 'bg-green-100 text-green-700' :
          ratingAvg >= 4.0 ? 'bg-blue-100 text-blue-700' :
          ratingAvg > 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'
        }`}>
          ...
          {reviews.slice(0, 3).map(r => (
            <div key={r.id} className="mt-4 pt-4 border-t border-gray-50">
              <div className="flex items-center gap-1 mb-1">
                {[1,2,3,4,5].map(i => <Star key={i} className={`w-3 h-3 ${i <= r.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'}`} />)}
                <span className="text-[10px] text-gray-400 ml-1">{new Date(r.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>
              </div>
      ...
      <div className="space-y-3">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Account standing</p>
        {warnings.map((w, i) => (
          <div key={i} className={`flex items-start gap-3 p-4 rounded-2xl border ${
            ...
            <p className={`text-sm leading-relaxed ${w.type === 'warning' ? 'text-amber-800' : 'text-green-800'}`}>
              {w.message}
            </p>
          </div>
        ))}
